...
#        df = pd.read_csv(os.path.join(self.raf_path, 'EmoLabel/list_patition_label.txt'), sep=' ', header=None)
        df = pd.read_csv(os.path.join(self.raf_path, 'generate_txt.txt'), sep=' ', header=None)
#        print(df)
        if phase == 'train':
            dataset=df
#            print(dataset)
...
#            dataset = df[df[NAME_COLUMN].str.startswith('test')]
            dataset=df[1000:2000]
#            print(dataset)
        file_names = dataset.iloc[:, NAME_COLUMN].values
#        print(file_names)
        self.label = dataset.iloc[:, LABEL_COLUMN].values  # 0:Surprise, 1:Fear, 2:Disgust, 3:Happiness, 4:Sadness, 5:Anger, 6:Neutral
