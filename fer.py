...
class FER2013(data.Dataset):
    ...
    def __init__(self, split='Training', transform=None):
        ...
        self.data = h5py.File('./data/data.h5','r',swmr = True)
#        train.file = open('data / fer2013 / train.txt'，'wb')
#        pickle.dump(Training_x，train_file)
        # now load the picked numpy arrays        （datapath，'w'，swmr = True）
#        print("---keys---", self.data.keys())                           
#        print("---key values---", self.data['Training_pixel'][:])  # slice for reading
