...
#print(opt.model)
#print('##############################################')
par = [''.join(list(g)) for k, g in groupby(opt.model, key=lambda x: x.isdigit())]
#print(par)
if par[0] == 'VGG':
    net = VGG(opt.model)
elif par[0]  == 'ResNet':
#    print("!!!!!!!!!!!")
    net = ResNet(opt.model)
elif par[0] == 'SeNet':
    net = SeNet(opt.model)
elif par[0] == 'DenseNet':
    net = Densenet(opt.model)
...
#criterion=nn.MSELoss(reduce="mean", size_average=True)
#optimizer = optim.SGD(net.parameters(), lr=opt.lr, momentum=0.9, weight_decay=5e-4)
#optimizer = optim.Adam(net.parameters(),weight_decay = 1e-4)
optimizer = optim.LBFGS(net.parameters(), lr=1, max_iter=20, max_eval=None, tolerance_grad=1e-05, tolerance_change=1e-09, history_size=100, line_search_fn=None)
...
#            % (train_loss/(batch_idx+1), 100.*correct/total, correct, total))
        print(epoch,train_loss/(batch_idx+1),float(100.*correct/total))
...
#        inputs, targets = Variable(inputs, volatile=True), Variable(targets)
        ...
        print(epoch,PrivateTest_loss/(batch_idx+1),float(100.*correct/total))
    # Save checkpoint.
